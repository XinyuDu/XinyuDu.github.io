...
\tag{1-9}
$$
需要注意的是式1-9等号右边的括号并不是运算符号，而是将函数和自变量分开，函数是$$A^3$$，括号内是函数则自变量取值。其中：
$$
O^3 表示第三层的输出，O^3=\begin{bmatrix}o^3_1\\o^3_2\end{bmatrix}\\
A^3表示第三层的激活函数，A^3=\begin{bmatrix}a^3_1\\a^3_2\end{bmatrix}\\
...
$$
上式表明第$$l-1$$层的输出组成的向量与连接第$$l-1$$和$$l$$层的权重矩阵相乘再加上$$l$$层的偏移量向量从而得到一个向量。将该向量的元素带入对应的第$$l$$层的激活函数中，从而得到第$$l$$层的输出。